...
namespace codicastudio\Statistics\Jobs;
...
class CrunchStatistics implements ShouldQueue
{
    ...
    {
        app('codicastudio.statistics.datum')->each(function ($item) {
            try {
                ...
                $route = app('codicastudio.statistics.route')->firstOrCreate([
                ...
                $agent = app('codicastudio.statistics.agent')->firstOrCreate([
                ...
                $device = app('codicastudio.statistics.device')->firstOrCreate([
                    'family' => $UAParser->device->family,
                    'model' => $UAParser->device->model,
                    'brand' => $UAParser->device->brand,
                ]);

                $platform = app('codicastudio.statistics.platform')->firstOrCreate([
                    'family' => $UAParser->os->family,
                    'version' => $UAParser->os->toVersion(),
                ]);

                $path = app('codicastudio.statistics.path')->firstOrCreate([
                ...
                $geoip = app('codicastudio.statistics.geoip')->firstOrCreate([
                ...
                app('codicastudio.statistics.request')->create($requestDetails);
                $item->delete();
